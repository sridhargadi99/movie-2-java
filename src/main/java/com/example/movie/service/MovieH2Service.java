...
public class MovieH2Service implements MovieRepository{
    ...
    public Movie addMovie(Movie movie){
        ...
        Movie movieDetails = db.queryForObject("SELECT * FROM MOVIELIST WHERE movieName = ? AND leadActor = ?", new MovieRowMapper(), movie.getMovieName(), movie.getLeadActor());
        return movieDetails;
    }
    ...
    public Movie getMovie(int movieId){
        try{
            Movie movieDetails = db.queryForObject("SELECT * FROM MOVIELIST WHERE movieId = ? ", new MovieRowMapper(), movieId);
            return movieDetails;
        }
        catch(Exception e){
